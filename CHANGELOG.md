...
- The final-review fix declares the exact runtime OpenAPI 503 `Retry-After` header schema, adds a
  ...
  approval and Subagent-Driven implementation authority. Human-review Draft PR is still pending.
- Published API `3.3.0-draft`, strict `OfficeListResponse`, generated shared contracts `0.6.0`, and
  ...
  `0.10.0-office-directory-runtime`; documentation is `2.20.9`.
...
- Bounded actual Docker/Supabase endpoint smoke is
  `Pending — local prerequisite unavailable` because no local secret environment or required
  process configuration was available; injected local integration passed. No `.env` was read/copied
  and no DB/provider process was started.
