...
- Published API `3.3.0-draft`, strict `OfficeListResponse`, generated shared contracts `0.6.0`, and
  test-suite `1.7.0-office-directory`. Application is
  `0.10.0-office-directory-runtime`; documentation is `2.20.8`.
- The aggregate verifier did not pass because it stopped at `PREFLIGHT-UV reason=exception code=2`
  after PowerShell/Node/pnpm preflights. All listed constituent gates passed: API 2,043 passed,
  8 DB-only skipped and 5 subtests passed with one pre-existing Starlette warning; shared contracts
  90/90; generation, docs, secret and diff checks passed.
- Bounded actual Docker/Supabase endpoint smoke is Pending because no local secret environment or
  required process configuration was available; injected local integration passed. No `.env` was
  read/copied and no DB/provider process was started.
