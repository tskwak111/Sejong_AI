...
/**
 * 이음센터 운영 현황(Overview) - DESIGN.md v3 §9-1.
 * KPI 카드 5개(목표 판정 뱃지) + 최근 실패 질문 패널 + 지표 안내.
 * 신규 기능·차트 절대 금지(P1) - P0 데이터의 뷰 재배치만. 실패 질문 mock 재사용.
 */
import { useCallback, useEffect, useState } from "react";
import Link from "next/link";
import { fetchKpi, fetchFailureQueue, type AdminKpi } from "@/lib/api";
import type { FailureQueueItem } from "@/types/api";
 ...
 * 목표가 없는 지표(총 질문 수·폴백률)는 뱃지 없이 캡션 "집계 지표"만 (최종 폴리시 12).
 */
const GOAL_ANSWER_RATE = 0.8;
const GOAL_RESPONSE_SEC = 3;
const GOAL_CITATION_RATE = 1.0;

function judgeKpi(kpi: AdminKpi): {
 ...
 * id는 KPI 스트립 지표명 탭 → 앵커 이동 대상 (모바일 정비 3).
 */
...
export default function AdminOverviewPage() {
  ...
  const [recent, setRecent] = useState<FailureQueueItem[] | null>(null);
  ...
  /** KPI 스트립 지표명 탭 → 지표 안내 해당 항목으로 스크롤 + 1회 하이라이트 (정비 3) */
  const goGuide = useCallback((id: string) => {
    setGuideFlash((prev) => ({ id, n: (prev?.n ?? 0) + 1 }));
  }, []);

  // 스크롤은 하이라이트 리마운트(key 변경) "이후"에 - effect에서 수행.
  // 즉시 이동(auto): §12 모션 허용 목록에 smooth 스크롤이 없고, 시선 유도는
  // 하이라이트 펄스가 담당한다
  useEffect(() => {
    if (!guideFlash) return;
    document
      .getElementById(guideFlash.id)
  ...
  const load = useCallback(async () => {
    setRefreshing(true);
    try {
      const [k, failures] = await Promise.all([
        fetchKpi(),
        fetchFailureQueue(),
      ]);
      setKpi(k);
      // 최신 5건 (실패 질문 화면과 동일 mock 데이터 재사용)
      setRecent(
        [...failures]
        ...
        {kpi === null ? (
          ...
          <div className="flex flex-col gap-5">
            {/* 1. KPI 카드 5개 - §9-1 목표 판정 뱃지.
                768px 미만은 한 줄 1지표 가로 스트립(간격 8px) - 5개 전체가
                최근 실패 질문 패널을 화면 밖으로 밀지 않게 (모바일 정비 2) */}
            ...
            {/* 2. 최근 실패 질문 패널 (§9-1) - 실패 질문 화면과 동일 mock 재사용 */}
            <section
              aria-label="최근 실패 질문"
            ...
            >
              ...
              <ul>
                {/* 768px 미만은 질문 1줄 + 메타(시각/사유) 1줄 세로 스택 -
                    128px 시각 열 + 사유 뱃지가 질문을 눌러 깨지는 것 방지 (정비 6) */}
                {(recent ?? []).map((f) => (
                  <li
                    key={f.id}
                  ...
                  >
                    ...
                    {f.masked_question !== null ? (
                      ...
                      <p className="text-note text-text-faint md:order-2 md:truncate">
                        {f.fallback_code === "INSUFFICIENT_GROUNDING"
                          ? "보관 기간 경과 (질문 텍스트 파기됨)"
                          : "텍스트 미보관 (분야와 사유만 집계)"}
                      </p>
                    )}
            ...
            {/* 3. 지표 안내 - 각 항목 id = KPI 지표명 앵커 대상. 이동 시 1회
                하이라이트(--color-highlight 펄스 재사용, 모바일 정비 3) */}
            <section
              aria-label="지표 안내"
            ...
            >
              ...
              <dl className="mt-3 space-y-1">
                {KPI_GUIDE.map(({ id, name, desc }) => (
                  ...
                  >
                    <dt className="text-admin-body font-bold text-text">
                      {name}
                    </dt>
