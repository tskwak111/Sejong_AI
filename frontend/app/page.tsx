 ...
 * 첫 뷰포트(390×844): 서비스명·소속 / 히어로 / 입력창 / 분야 카드 상단 걸침.
 */
...
import { CATEGORY_LABEL, type CivilCategory } from "@/types/api";

/* 분야별 예시 질문 칩 2개 - 탭하면 해당 질문으로 대화 시작 (§8-4, SFR-006) */
const CATEGORY_CHIPS: Record<CivilCategory, string[]> = {
  MOVE_IN: ["전입신고는 언제까지 해야 하나요?", "이사했는데 뭐 해야 하나요?"],
  BULKY_WASTE: [
  ...
  LOCAL_TAX: ["자동차세는 언제 내나요?", "제 자동차세 얼마 나왔나요?"],
};
...
const CATEGORY_ICON_PATHS: Record<CivilCategory, React.ReactNode> = {
  MOVE_IN: (
    <>
...
function CategoryIcon({ category }: { category: CivilCategory }) {
  return (
    <svg
      aria-hidden="true"
      className="h-6 w-6"
      ...
      <main id="main" className="flex-1">
        ...
        >
          ...
          <ul className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
            {(Object.keys(CATEGORY_LABEL) as CivilCategory[]).map((key) => (
              <li
                key={key}
