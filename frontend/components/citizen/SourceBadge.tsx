 ...
 * 상단 뱃지가 쓰던 verify 문법을 승계: --verify-tint 배경 + --verify-border
 * 상단 보더 + 초록 체크 아이콘. 구성(좌→우): 아이콘 + "공식 출처 확인" +
 * 문서명(semibold) + 확인일 + 원문 보기 링크.
 * KB ID는 시민 화면 어디에도 표시하지 않는다 (최종 폴리시 8) -
 * 데이터(types/api.ts Source.kb_id)와 이음센터 표기는 유지.
      ...
      <ul className="space-y-1">
        {sources.map((s, i) => (
          ...
          >
            ...
            {/* 최종 폴리시 8: KB ID는 시민 화면 미표시 (데이터·이음센터에는 유지) */}
