 ...
 *
 * 태성 리뷰 1: 질문 전달은 `/chat?q=` 쿼리 대신 탭 메모리(pending-question)
 * 로만 한다 - 질문 원문이 URL·히스토리·서버 로그에 남지 않는다.
 * 태성 리뷰 2: fixture 모드에서는 최상단 앰버 배너를 상시 노출한다.
 */
...
export default function HomeScreen({
  ...
  /** CHAT_UI_MODE=fixture 명시 설정 시에만 true - 시연용 샘플 배너 노출 */
  fixtureMode?: boolean;
}) {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (!q) return;
    // 태성 리뷰 1: 질문 원문은 탭 메모리로만 전달 - URL 쿼리 금지
                  ...
                  {CATEGORY_CHIPS[key].map((q) => (
                    <li key={q}>
                      {/* 예시 질문 칩 - min-height 52px, 전체 폭, outline.
                          질문 원문은 탭 메모리로 전달, href에 쿼리 없음 (태성 리뷰 1) */}
                      <Link
                        href="/chat"
