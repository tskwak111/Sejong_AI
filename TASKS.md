...
| OFFICE-API-001 | P1 | Backend·Contract·QA | declared `GET /api/v1/offices`와 FastAPI runtime 정합 / [실행계획](docs/superpowers/plans/2026-07-26-office-api-runtime-parity.md) / [구현 노트](docs/implementation-notes/IMP-20260726-012-office-api-runtime-parity.md) | Done — Subagent-Driven implementation complete; human-review Draft PR pending | CONTRACT-001, DB-001, Q-API-OFFICES-001=A/D-078/D-080 | default route discovery와 local injected OFFICIAL-only `public_id` order, match/valid-empty 200, value-free 422, runtime/tracked 503 `Retry-After` schema 정합을 구현했다. aggregate `scripts/verify.ps1`은 NOT PASS이고 constituent API/shared/docs/secret/diff gate는 PASS; actual Docker/Supabase smoke는 `Pending — local prerequisite unavailable`. DB/data/Web/LLM/dependency/public/remote 변경 0 |
