...
| OFFICE-API-001 | P1 | Backend·Contract·QA | declared `GET /api/v1/offices`와 FastAPI runtime 정합 | Design Approved / written specification Review | CONTRACT-001, DB-001, Q-API-OFFICES-001=A/D-078 | existing required region+intent, OFFICIAL-only, public-id order와 valid empty 200을 runtime에 구현하고 closed/readiness/DB 불능은 safe 503으로 처리한다. DB migration·seed·Web·LLM·dependency·public/remote 변경 0; spec/plan 승인 뒤 TDD |
