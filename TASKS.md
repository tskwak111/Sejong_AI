...
| OFFICE-API-001 | P1 | Backend·Contract·QA | declared `GET /api/v1/offices`와 FastAPI runtime 정합 | Pending — status audit finding | CONTRACT-001, DB-001 | OpenAPI에는 path가 있으나 current default/local FastAPI router에는 없다. P0 chat의 server-bound office card는 완료됐으므로 read-only endpoint 구현 또는 계약 정리는 공개 계약 검토 뒤 진행한다 |
