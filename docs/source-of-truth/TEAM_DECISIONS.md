...
- Q-LLM-006~012/D-072: 사용자는 이후 local/private 입찰 시연 MVP에 한해 실제 시민 chat의
  ...
  SUCCESS는 `GENERATED|TEMPLATE` 작성 방식 배지를 제공한다. written specification과 계획
  승인 전 구현하지 않으며 public/remote/실제 기관 운영은 계속 금지한다.
...
- 7월 25일 뒤 외부 LLM은 Q-LLM-005=A 합성 평가로 시작했고 D-071에서 FAIL로 종료했다.
  이후 Q-LLM-006~012/D-072가 local/private 근거 제한형 시민 chat 설계를 승인했다. written
  specification과 계획 승인 전에는 구현하지 않는다. 고급 UI polish, 100명 부하, 자동 백업,
...
- MVP-001은 PR #9 병합으로 local/private AI scope **Done**이다. PR #6과 Frontend PR #8은 사람이
  ...
  written specification·계획 승인 전 구현 상태는 변하지 않는다. 100-user,
