...
| SFR-002 | 민원 의도 분류 | 전입·주민등록, 증명서, 대형폐기물, 지방세, 범위 밖과 topic/coverage를 closed enum으로 분류 | P0 실제 구현 + selectable classifier; A-080 transport/wire verified, strict quality FAIL 8/9; exact-field post-actual fix offline | frozen classifier 60/60, Hybrid UAT 48/48; A-080 response/2xx/strict/accepted9/9/9/9, oracle8/9; post-actual prompt22·shared-provider133 PASS | 모호 질문은 exact FOLLOWUP, topic 없음은 INSUFFICIENT_GROUNDING. A-080은 exact-one 소비·재실행 금지이며 post-actual `intent=supported` 교정은 live quality 미검증이다. 시민 runtime은 deterministic fallback, final answer 생성은 Upstage 유지 |
...
| DAR-004 | 시민 의견 | 답변 request ID별 만족/불만족과 선택형 분야·사유, 기타 상세 300자 | P1 실제 구현 | API/Web 저장 성공·실패, 관리자 집계, pgTAP 30일 파기·멱등 충돌 | raw 상세·질문·답변 미저장; 상세는 서버 마스킹 후 30일만 보관 |
