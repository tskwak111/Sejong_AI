...
| SER-001 | 개인정보 최소수집 | 외부 LLM 호출 전 마스킹, 원문 DB 미저장, 앱 DB IP·기기ID 미수집 | P0 실제 구현 + LLM-003 D-074 In Progress | 데이터 흐름·스키마·로그·provider gate·payload 최소화와 local DB exact single-loopback 검사 | local/private 시민 chat은 D-072~D-074 승인 gate만; public/remote/실제 기관 운영 금지. DB tooling은 D-031/D-032의 pinned patched CLI와 short checkout path-budget을 통과해야 함 |
