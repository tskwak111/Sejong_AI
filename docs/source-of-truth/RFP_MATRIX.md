...
| --- | --- | --- | --- | --- | --- |
| SFR-001 | 자연어 질의응답 | 4개 우선 분야의 일상어 질문을 공식 ACTIVE KB에 연결하고 구조화 답변과 출처 제공 | P0 실제 구현 + LLM-003 offline implementation/final root gate complete / actual Pending | 정상 질문 10개에서 분류·근거·구조화 필드·출처와 generated/template fallback 확인 | Upstage `solar-pro3`는 local/private에서 supported+masked+ACTIVE/OFFICIAL+grounded일 때만 사용; server-issued fact ID 검증 실패 시 전체 template, public/remote 금지 |
...
| SER-001 | 개인정보 최소수집 | 외부 LLM 호출 전 마스킹, 원문 DB 미저장, 앱 DB IP·기기ID 미수집 | P0 실제 구현 + LLM-003 offline implementation/final root gate complete / actual Pending | 데이터 흐름·스키마·로그·provider gate·payload 최소화와 local DB exact single-loopback 검사 | local/private 시민 chat은 D-072~D-074 승인 gate만; public/remote/실제 기관 운영 금지. DB tooling은 D-031/D-032의 pinned patched CLI와 short checkout path-budget을 통과해야 함 |
