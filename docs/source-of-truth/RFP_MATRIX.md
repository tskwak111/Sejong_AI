...
| SFR-002 | 민원 의도 분류 | 전입·주민등록, 증명서, 대형폐기물, 지방세, 범위 밖과 topic/coverage를 closed enum으로 분류 | P0 실제 구현 + selectable classifier; A-077 split-timeout verification in progress | frozen classifier 60/60, Hybrid UAT route/intent/topic 48/48, A-076 offline/readiness/network probe PASS but actual transport-no-response9; A-077 related-area 153 PASS before external evidence | 모호 질문은 exact FOLLOWUP, topic 없음은 INSUFFICIENT_GROUNDING. local/private selector만 허용하고 final answer 생성은 Upstage 유지. A-077은 connect3/read+complete10초의 1-call 2xx probe 뒤에만 9-call actual을 허용한다 |
