...
| A-072 | B / High | provider가 exact five-key closed object를 반환하도록 만드는 다음 단일 교정 | Pending — design/human gate | A-071은 JSON object까지는 성공하지만 exact key set에서 9/9 거절됨을 증명했다. body를 보관하거나 validator를 완화하지 않고 prompt 또는 provider-supported structured schema 중 한 변수를 선택해 offline TDD한 뒤, 새 actual은 별도 인간 승인을 받아야 한다. | D-111, ADR-0027; public behavior/API/DB/data 불변 |
