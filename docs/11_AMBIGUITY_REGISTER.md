...
| A-051 | B / High | declared office endpoint 존치와 runtime 정합 | Resolved design / Q-API-OFFICES-001=A | existing `GET /api/v1/offices`를 required region+supported intent, OFFICIAL-only, deterministic order와 safe 503으로 FastAPI에 구현한다. | D-078 / OFFICE-API-001 design. written specification과 실행계획 승인 전 product code 0; DB/data/Web/LLM/dependency/public/remote 불변 |
