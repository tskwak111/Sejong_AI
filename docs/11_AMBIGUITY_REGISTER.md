...
| A-060 | A / Blocker | hybrid 분류와 grounded generation의 provider 호출·비용 상한 | Pending Q-CLASS-002 | ambiguous supported 질문은 classifier 1회 뒤 answer generation 1회로 최대 2 outbound가 가능하다. 기존 generation cap 30을 그대로 공유하면 starvation, 무제한 분리하면 비용 증가 위험이 있다. | 전체 process cap과 timeout/output 한도를 인간 승인하고 local synthetic acceptance 전 actual classifier call 금지 |
