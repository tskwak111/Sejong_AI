...
| A-051 | B / High | declared office endpoint 존치와 runtime 정합 | Resolved specification / execution plan Review | existing `GET /api/v1/offices`를 required region+supported intent, OFFICIAL-only, deterministic order와 safe 503으로 FastAPI에 구현한다. | D-078/D-079 / OFFICE-API-001 spec·plan. 실행계획 승인 전 product code 0; DB/data/Web/LLM/dependency/public/remote 불변 |
