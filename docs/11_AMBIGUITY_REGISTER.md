...
| A-072 | B / High | provider가 exact five-key closed object를 반환하도록 만드는 다음 단일 교정 | Design section 1 Approved — section 2 Review | Q-LLM-014=A/D-112와 D-113으로 strict `json_schema`, required string 5필드, nullable 4필드의 exact `NONE` normalization, server-owned closed validation을 승인했다. public behavior/API/DB/data/dependency는 불변이며 section 2와 written spec 승인 전 code/provider call 0, corrective actual은 구현·offline 검증 뒤 별도 인간 gate다. | D-111~D-113, ADR-0027, IMP-20260728-012~014 |
