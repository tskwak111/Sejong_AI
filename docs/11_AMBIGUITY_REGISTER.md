...
|---|---|---|---|---|---|
| A-001 | A | LLM | Resolved historically / original provider choice superseded | 최초 합성 evaluator의 DeepSeek 선택은 D-065 Upstage로 대체됐다. 현재 local/private classifier 선택 권위는 별도 A-074/D-122/ADR-0028이며, 과거 합성 max 1024·concurrency 1·retry 1·run당 outbound 30 경계와 혼동하지 않는다. | Q-LLM-004 / D-023 / ADR-0005/0022; current classifier A-074/D-122 |
...
| A-072 | B / High | provider가 exact five-key closed object를 반환하도록 만드는 다음 단일 교정 | Resolved — exact key-set correction verified; overall actual FAIL | D-117 exact-one actual은 20/0/11/9, HTTP 2xx·usage·stage 9를 기록했고 D-111의 `KEY_SET_REJECTED`는 9→0으로 해소됐다. 그러나 9건 모두 `ENUM_SHAPE_REJECTED`, accepted/match 0이라 전체 acceptance는 FAIL이다. 재실행하지 않았고 시민 runtime은 fail-closed다. | D-111~D-117, current D-117 report, archived D-111 report, IMP-20260729-001 |
| A-073 | B / High | exact five-key 응답 9건이 enum/shape 검증에서 모두 거절된 원인과 다음 bounded 교정 | Resolved — offline scoped review clean; root aggregate NOT VERIFIED/FAIL preserved; actual unexecuted | D-120 승인으로 shared builder·five refined stages·explicit route matrix·literal `NONE`·intent-grouped catalog·wire oracle를 TDD 구현했고 D-121 final review에서 네 provider intent vocabulary, five adjacent first-failure precedence와 selected-question/provider-body/invalid-value 비보관을 보강했다. final prompt 4,067자·margin 29, area 397·controlled-double 39·Ruff/Mypy 115가 PASS했고 scoped re-review finding은 0이다. Task 5 root wrapper exact 1회는 harness timeout 124로 final stdout/exit를 회수하지 못해 aggregate를 `NOT VERIFIED/FAIL`로 기록했으며 rerun은 0이다. 독립 docs/secret/diff/status는 PASS, A-073 provider actual은 0이고 기존 Upstage actual도 재실행하지 않았다. | D-117~D-121, ADR-0027, A-073 approved specification/plan, IMP-20260729-003/004/005 |
| A-074 | A / Blocker | local/private 질문 분류 비교 공급자, exact trust boundary, 비용·보관·one-shot actual 범위 | Resolved / Closed offline — immutable gate FAIL; actual blocked | Q-LLM-PROVIDER-001=A/D-122로 DeepSeek exact `deepseek-v4-flash`를 selectable classifier-only provider로 확정하고 Tasks 1~6b·pre-gate C0/I0/M0를 완료했다. D-123 source `9c7f818...` exact-one gate는 exit1·timed_out false·invocation/rerun1/0·first stage `TEST-ROOT`로 immutable FAIL이다. Standalone 434-test repository-truth mismatch는 test-only +4 교정 뒤 `434 OK/skipped2`·review C0/I0/M0지만 gate를 소급 변경하지 않는다. Exact five-string/`NONE`, strict JSON/bounded response/deadline/identity, deterministic safety·ACTIVE/OFFICIAL grounding과 Upstage final generator를 유지한다. DeepSeek actual은 blocked/unexecuted 0/0, report/lease·outbound·token·cost 0이며 public/remote/실제 시민 운영은 금지다. | D-122/D-123, ADR-0028, immutable A-074 result/hash evidence, plan/runbook/IMP-006 |
| A-075 | A / Blocker | A-074 immutable FAIL 뒤 별도 DeepSeek corrective actual 권한과 identity | Resolved / actual FAIL — transport no response | Source `982198f...`의 A-075 offline/readiness는 PASS했지만 actual outbound9 모두 HTTP 응답 전 `transport_no_response`, provider response/2xx/parse/accepted/match/token0으로 FAIL했다. 20/0·11/9·privacy/policy outbound0, retention/retry/rerun0과 permanent report/lease를 보존한다. 하위 transport 원인은 단정하지 않으며 추가 actual은 새 승인 전 금지다. | D-124/D-125, ADR-0028, A-075 report/IMP-007 |
| A-076 | A / Blocker | A-075 transport FAIL 뒤 네트워크 복구 확인과 별도 actual 권한 | Resolved / actual FAIL — likely 3s timeout | Value-free network probe와 offline/readiness는 PASS했지만 exact-one actual outbound9 모두 HTTP 응답 전 실패했다. 28.6초는 9×3초 timeout과 거의 일치하나 하위 exception은 비보관이라 확정하지 않는다. Retention/retry/rerun0과 permanent report/lease를 보존하며 timeout 변경·추가 actual은 새 결정 전 금지다. | D-126/D-127, ADR-0028, A-076 report/specification/plan, IMP-008 |
| A-077 | A / Blocker | A-076 3초 전체 timeout 가설의 최소 교정과 추가 actual 권한 | Resolved / offline PASS retained; provider execution superseded | Q-LLM-015=A/D-128로 connect/write/pool3초·read/complete10초·retry0을 확정했다. Source `675eef4...` offline은 PASS 1/0이나 review Important 3건 때문에 probe/actual 0회 상태에서 A-078로 승계했다. A-077 evidence는 불변 보존한다. | D-128/D-129, ADR-0028 amendment, A-077 specification/plan, IMP-009 |
| A-078 | D / Internal | A-077 probe-to-actual same-source·exact-lease TOCTOU 보강 | Resolved / probe FAIL; actual blocked | Source `844e53b...` offline PASS 1/0 뒤 probe outbound1은 HTTP 응답 없이 transport-no-response1로 FAIL했다. Retry/rerun/retention0이고 actual report/lease는 0이다. CRLF lease defect는 원인과 분리해 binary-open으로 교정하며 A-078은 재실행하지 않는다. | D-129/D-130, A-078 probe report, IMP-009 |
| A-079 | A / Blocker | A-078 당시 네트워크 이상 가능성 뒤 별도 retry 권한 | Resolved / transport+contract PASS, quality FAIL 6/9 | Source `a2d617c...` offline PASS 뒤 probe 1/1 2xx PASS, actual outbound/response/2xx/strict/accepted9/9/9/9/9로 연결·계약은 성공했다. Oracle match6/9라 overall FAIL이며 retry/rerun/retention0이다. 추가 품질 교정 actual은 새 인간 결정 전 금지한다. | D-131/D-132, A-079 reports, IMP-010 |
| A-080 | B / High | A-079 oracle 6/9의 provider-neutral 품질 교정과 one-shot actual gate | Resolved / actual transport+wire PASS, quality FAIL 8/9; post-actual exact-field fix offline | D-136 source `f2c3aec...` offline/readiness PASS 뒤 exact-one actual은 20/0·11/9, privacy/policy outbound0, response/2xx/strict/accepted9/9/9/9와 oracle8/9를 기록했다. D-137은 executed-source ancestry를 보존하고 undefined `I`를 exact `intent`로 TDD 교정했지만 post-actual source라 live 품질은 미검증이다. Retry/rerun/runtime/retention0, cost USD0.002961266<0.20이며 default 승격·재실행은 금지한다. | D-133~D-137, ADR-0028 eighth amendment, A-080 report/specification/plan, IMP-011~014 |
| A-081 | A / Blocker | 시민 자유입력 피드백의 개인정보 마스킹·보관·관리자 반영 | Resolved / Q-FEEDBACK-001=A, implementation approved | 분야/사유 OTHER와 최대 300자 상세를 허용하되 서버 마스킹 뒤 masked text만 30일 보관한다. raw 상세·질문·답변은 저장하지 않고 unresolved는 no-row로 닫는다. local/private 관리자 aggregate만 허용하며 public abuse/rate-limit/auth는 후속 gate다. | D-138 / ADR-0029 / feedback approved spec+plan |
