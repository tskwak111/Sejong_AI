...
| A-070 | B / High | Upstage 4xx class의 정확한 운영 원인과 다음 교정 방식 | Investigated / Q-LLM-013 pending | 사용자가 key 목록 존재·credit 있음·`solar-pro3` 선택 가능을 확인했다. Usage 최신 시각은 corrective run보다 이전이라 해당 4xx는 usage에 반영되지 않았다. working historical prompt에는 명시적 `JSON` 지시가 있었지만 current 9/9 prompt에는 0이고 request는 `response_format=json_object`를 유지한다. 이 JSON-mode prompt 회귀가 1순위 가설이나 actual로 확정하지 않았다. | Q-LLM-013에서 explicit JSON instruction TDD와 정확히 1회 PII-free corrective actual 여부 결정; 새 승인 전 provider call 0 |
