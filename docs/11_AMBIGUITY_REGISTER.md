...
| A-071 | B / High | 2xx 응답 9건이 strict closed decision 전에 모두 거부된 정확한 단계 | Design approved / written spec confirmation pending | 사용자가 추천 A인 production parser typed optional observer를 승인했다. terminal fixed enum 하나만 aggregate count로 전달하고 body/status/exception/key/DSN은 0이다. same fixed 20·9 outbound·retry 0·USD0.20 exact-one actual 방향도 승인됐으며 written spec 확인 전 code/provider call은 0이다. | D-108, `2026-07-28-upstage-classifier-value-free-response-stage-diagnostics-design.md` |
