...
| A-073 | B / High | exact five-key 응답 9건이 enum/shape 검증에서 모두 거절된 원인과 다음 bounded 교정 | Design approved / written specification Review | D-118은 prompt의 compact `default=NONE`/불완전 route 조합을 제거하고 exact route matrix·literal `NONE`·same-row topic/coverage를 명시하는 A안을 선택했다. generic enum/shape stage는 route·intent·pending-slot·identifier·route-shape의 value-free first-failure aggregate로 세분화한다. 기존 body·질문·wrong value·fixture별 stage 비보관과 fail-closed를 유지한다. 구현은 spec/plan 승인 뒤, actual은 별도 exact 인간 승인 뒤에만 가능하다. | D-117/D-118, ADR-0027, A-073 written specification, IMP-20260729-003 |
