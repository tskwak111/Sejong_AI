...
| A-073 | B / High | exact five-key 응답 9건이 enum/shape 검증에서 모두 거절된 원인과 다음 bounded 교정 | Open — new human decision required before implementation or actual | aggregate-only evidence로는 `ENUM_SHAPE_REJECTED` 9/9까지만 확정한다. provider body·질문·status detail을 보관하지 않았으므로 route/intent enum 또는 nullable field 조합 중 무엇이 어긋났는지 단정하지 않는다. 다음 단계는 content-free 진단 설계 또는 provider 선택 비활성 유지를 비교해야 하며 새 actual은 별도 인간 승인 전 금지한다. | D-117, ADR-0027, current actual report |
