...
| A-053 | B / High | generic 증명서 FOLLOWUP 반복 | Pending / root cause actual reproduced | Q-CHAT-FOLLOWUP-001: `증명서 발급해야해` 뒤 `증명서 발급` option을 선택해도 UNKNOWN 네 분야 FOLLOWUP이 반복된다. | category-aware `CERTIFICATE_ISSUANCE + FOLLOWUP`과 ACTIVE CERT 5개 범위 option을 추천 |
