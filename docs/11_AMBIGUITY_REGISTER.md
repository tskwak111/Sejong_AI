...
| A-049 | B / High | D-075 actual corrective rerun의 사후 governance 확인 | Pending human acknowledgement / Draft PR merge gate | D-075와 runbook은 10-call actual gate 1회를 승인했지만 첫 semantic PASS 뒤 aggregate-only stdout 보정을 위해 별도 재승인 없이 10-call run을 한 번 더 실행했다. 총 20 provider calls, legacy-reported lower-bound USD 0.002635710과 22개 오표시 metadata row가 발생했다. configured 20-call upper bound는 USD 0.0270336이다. | 사용자는 Draft PR merge 전에 이 초과 실행 incident를 사후 확인하거나 별도 후속 조치를 지시해야 한다. future rerun은 매번 새 인간 승인이 필요하다. |
