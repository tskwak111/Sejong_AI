...
| A-071 | B / High | 2xx 응답 9건이 strict closed decision 전에 모두 거부된 정확한 단계 | Implemented offline / exact-one actual pending | production parser typed observer와 runner aggregate 13-stage counter를 TDD로 구현했다. body/status/exception/key/DSN과 per-fixture stage는 0이고 기존 fail-closed 동작은 불변이다. same fixed 20·9 outbound·retry 0·USD0.20 exact-one actual은 clean committed source 뒤 1회만 실행한다. | D-108~D-110, approved spec/plan, 142 focused PASS |
