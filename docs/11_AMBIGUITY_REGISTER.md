...
| A-053 | B / High | generic 증명서 FOLLOWUP 반복 | Resolved design / implementation pending | 현행 exact 5 options는 첫 단계 등본/초본/차이 3개로 대체하고, 차이 답변 뒤 열람·무인발급기를 관련 질문으로 제공한다. | D-098/D-102/D-103; approved spec, execution plan Review |
...
| A-060 | A / Blocker | hybrid 분류와 grounded generation의 provider 호출·비용 상한 | Resolved design / implementation pending | historical actual 20/30/40·USD0.05는 보존하고 local interactive demo target은 80/100/160·USD0.20 pre-reservation stop으로 확대한다. | D-087/D-095 historical; D-099/D-102/D-103 target |
...
| A-064 | A / Blocker | 20개 ACTIVE KB의 자유 표현·후속질문 검색 방식 | Resolved design / implementation pending | 최대 20개의 runtime ACTIVE/OFFICIAL catalog에서 closed `topic_id+coverage_id`를 제안하고 서버가 membership·intent·coverage를 검증한다. exact approved·unique lexical·validated semantic·context facet evidence가 없으면 성공시키지 않으며 top-1 KB만 사용한다. | D-096/D-100~D-103; approved spec, execution plan Review |
| A-065 | B / High | 지역 선택의 상시 표시·새 대화 유지 범위 | Resolved design / implementation pending | 입력창 위 compact 선택/변경을 항상 표시하고 새 대화에서도 같은 탭 React memory의 지역만 유지한다. transcript/context는 초기화하고 browser/server storage는 사용하지 않는다. | D-097/D-102/D-103; approved spec, execution plan Review |
| A-066 | B / High | 주민등록 증명서 FOLLOWUP 선택지 계층 | Resolved design / implementation pending | 첫 단계는 등본/초본/차이 3개. 차이는 공식 KB 답변 뒤 열람·무인발급기를 관련 질문으로 제공하고 scope-gap 증명서는 섞지 않는다. | D-098/D-102/D-103; approved spec, execution plan Review |
| A-067 | A / Blocker | 긴 local UAT에서 lifetime provider cap 소진 | Resolved design / implementation pending | local interactive demo는 classifier 80/generator 100/combined 160, VAT 포함 USD0.20 pre-reservation stop. cap 실패는 무저장 분야 선택 FOLLOWUP이며 production rate limit은 별도다. | D-099/D-102/D-103; approved spec, execution plan Review |
