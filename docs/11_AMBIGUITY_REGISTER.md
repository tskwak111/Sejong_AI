...
| A-053 | B / High | generic 증명서 FOLLOWUP 반복 | Resolved decision / written specification Approved / plan Review; integration paused | Q-CHAT-FOLLOWUP-001=A/D-083/D-084: generic `증명서`는 supported CERT FOLLOWUP과 exact approved 5 options를 반환하고 선택 후 ACTIVE-only retrieval을 사용한다. | A-057~A-059의 상위 classification taxonomy 확정 뒤 같은 classifier/service 파일의 통합 계획으로 RED→GREEN 구현 |
| A-054 | B / High | 일반 관리자 KB 후보 작성 | Resolved requirement / implementation gap | current actual draft builder는 exact WASTE-03 fixture만 허용하고 다른 eligible IG failure를 거부한다. LLM 연결과 무관하며 일반 eligible failure의 운영자 작성→별도 승인은 이미 P0다. | arbitrary eligible failure용 운영자 구조화 작성 폼과 기존 검증/저장 capability를 연결하며 공식 필드는 사람이 작성 |
| A-055 | B / High | 시민 최초 지역 선택 진입점 | Resolved requirement / implementation gap | office endpoint는 OFFICIAL-only로 정상이나 initial chat에서 `selected_region`을 만들 수 없어 기관 카드에 도달하기 어렵다. 읍·면·동 직접 선택은 이미 P0다. | `/chat` 상단의 직접 읍·면·동 선택을 구현하고 office card E2E로 검증 |
...
| A-058 | A / Blocker | local/private 질문 분류의 LLM 경계 | Resolved direction / Q-CLASS-001=A / implementation blocked by spec | PII·policy·명백한 분류는 deterministic, 안전한 ambiguous 질문만 bounded strict-enum Upstage 분류에 사용한다. 모델은 답변·출처·저장 여부를 결정하지 못하며 fail closed한다. | D-086/ADR-0025. exact schema·timeout·budget·fallback은 written spec; actual call은 Q-CLASS-002 전 금지 |
| A-059 | A / Blocker | 현재 네 분야 밖 행정 민원의 저장·검토 정책 | Resolved design / implementation blocked by spec | public `intent=OUT_OF_SCOPE`, reason `CIVIC_SCOPE_GAP`, candidate false. 별도 queue에 PII-safe masked text만 30일 보관하고 event/failed/candidate와 분리·자동 ACTIVE 금지. NON_CIVIC은 text/event/review row 0 | D-085/D-090/ADR-0024. exact table/status/endpoint와 migration/rollback/pgTAP은 written spec·계획 승인 뒤 구현 |
| A-060 | A / Blocker | hybrid 분류와 grounded generation의 provider 호출·비용 상한 | Resolved / Q-CLASS-002=A / implementation blocked by spec | classifier 1회·3초·retry 0·입력 1,024자·출력 128 token·sub-cap 20, generation 1회·8초·sub-cap 30, combined process cap 40, local synthetic cost stop line VAT 포함 USD 0.05 | D-087/ADR-0025. hidden retry/reset 금지; exact schema·계획·별도 local actual 승인 전 classifier call 0 |
