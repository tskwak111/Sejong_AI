...
| A-073 | B / High | exact five-key 응답 9건이 enum/shape 검증에서 모두 거절된 원인과 다음 bounded 교정 | Implemented offline / Task 5 root aggregate NOT VERIFIED; actual blocked | D-120 승인으로 shared builder·five refined stages·explicit route matrix·literal `NONE`·intent-grouped catalog·wire oracle를 TDD 구현했다. area 386·controlled-double 39·Ruff/Mypy 115가 PASS했고 provider/network actual call·cost는 0이다. baseline-stale mock JSON null 1건은 RED 뒤 exact wire `"NONE"`으로만 교정했다. Task 5 root wrapper exact 1회는 harness timeout 124로 final stdout/exit를 회수하지 못해 aggregate를 FAIL로 기록하고 재실행하지 않았다. 독립 docs/secret/diff/status는 PASS다. body·질문·wrong value·fixture별 stage 비보관과 fail-closed를 유지하며, actual은 root-gate 해소와 exact 별도 인간 승인 뒤에만 가능하다. | D-117~D-120, ADR-0027, A-073 approved specification/plan, IMP-20260729-003/004/005 |
