...
| A-072 | B / High | provider가 exact five-key closed object를 반환하도록 만드는 다음 단일 교정 | Interviewing — Q-LLM-014 pending | A-071은 JSON object까지는 성공하지만 exact key set에서 9/9 거절됨을 증명했다. current request는 `json_object`만 보내며 공식 Upstage exact 구조는 strict `json_schema`를 사용한다. 공식 타입 목록에 null/union이 명시되지 않아 required string five-key + `NONE` sentinel(A, 추천), nullable union(B), prompt-only(C) 중 인간 결정을 받는다. body/validator 완화는 0이고 새 actual은 별도 승인 대상이다. | D-111, ADR-0027, IMP-20260728-012; public behavior/API/DB/data 불변 |
