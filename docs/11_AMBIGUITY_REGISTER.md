...
| A-053 | B / High | generic 증명서 FOLLOWUP 반복 | Resolved design / written specification Review | Q-CHAT-FOLLOWUP-001=A/D-083: generic `증명서`는 supported CERT FOLLOWUP과 exact approved 5 options를 반환하고 선택 후 ACTIVE-only retrieval을 사용한다. | 명세 검토와 계획 승인 뒤 classifier/service/response/Web을 TDD 구현 |
