...
| A-072 | B / High | provider가 exact five-key closed object를 반환하도록 만드는 다음 단일 교정 | In Progress — Tasks 1~5 offline/clean-source review complete; actual Pending | D-116으로 plan과 Subagent-Driven 구현을 승인했다. provider-only exact `NONE` parser, bounded canonical prompt와 fresh strict schema를 구현했다. Task 5 root wrapper는 단 1회 실행해 environment-only `PREFLIGHT-UV`에서 FAIL했고 재실행하지 않아 PASS가 아니다. 나머지 constituent·security·scope 검사는 bounded skip과 함께 PASS했고 provider call/cost는 0이다. Task 6 actual은 D-117 exact 문구 `A-072 corrective actual 1회 실행 승인` 전까지 금지한다. | D-111~D-116, ADR-0027, A-072 spec/plan, IMP-20260728-012~017 |
