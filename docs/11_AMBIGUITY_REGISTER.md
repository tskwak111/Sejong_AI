...
| A-051 | B / High | declared office endpoint 존치와 runtime 정합 | Resolved / execution plan approved; Subagent-Driven implementation complete; human-review Draft PR pending | existing `GET /api/v1/offices`를 required region+supported intent, OFFICIAL-only, deterministic order와 safe 503으로 FastAPI에 구현했다. | D-078/D-080 / OFFICE-API-001 spec·plan. DB/data/Web/LLM/dependency/public/remote 불변 |
