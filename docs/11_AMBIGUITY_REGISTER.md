...
| A-079 | A / Blocker | A-078 당시 네트워크 이상 가능성 뒤 별도 retry 권한 | Resolved / implementation in progress | D-131로 binary-open correction이 포함된 새 clean source에서 offline→readiness→probe 1-call→2xx일 때만 actual run 1회(9 provider calls)를 허용했다. A-078 evidence는 불변이고 자동 rerun은 없다. | D-131, ADR-0028 third amendment, A-079 plan, IMP-010 |
