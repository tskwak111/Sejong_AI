...
| A-074 | A / Blocker | local/private 질문 분류 비교 공급자, exact trust boundary, 비용·보관·one-shot actual 범위 | Resolved / offline implementation complete; exact-one gate and actual pending | Q-LLM-PROVIDER-001=A/D-122로 DeepSeek exact `deepseek-v4-flash`를 selectable classifier-only provider로 확정했고 Tasks 1~6 offline TDD·review를 완료했다. exact five-string/`NONE`, shared server parser, deterministic PII/policy/obvious routes, ACTIVE/OFFICIAL server grounding과 기존 Upstage final generator를 유지한다. local `create_local_app`/loopback만 허용하고 public/remote/실제 시민 운영은 금지한다. 새 A-074 offline wrapper와 clean-source review 뒤 고정 20 actual lease를 1회만 소비하며 PASS/FAIL과 무관하게 rerun0이다. 현재 gate/actual invocation은 각각 0이다. | D-122, ADR-0028, A-074 discovery/spec/plan/runbook; final status becomes actual PASS or FAIL evidence |
