...
| A-070 | B / High | Upstage 4xx class의 정확한 운영 원인과 다음 교정 방식 | Approved / corrective actual pending | 사용자가 key 목록 존재·credit 있음·`solar-pro3` 선택 가능을 확인했고 Q-LLM-013=A로 명시적 `JSON만` 지시의 TDD 복원과 정확히 1회 PII-free corrective actual을 승인했다. route·catalog·model·timeout·retry는 고정하며 실패해도 재시도하지 않는다. | D-107; prompt/test GREEN 뒤 committed exact source에서 aggregate-only actual 1회 |
