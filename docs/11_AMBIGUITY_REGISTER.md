...
| A-053 | B / High | generic 증명서 FOLLOWUP 반복 | Resolved decision / written specification Approved / plan Review | Q-CHAT-FOLLOWUP-001=A/D-083/D-084: generic `증명서`는 supported CERT FOLLOWUP과 exact approved 5 options를 반환하고 선택 후 ACTIVE-only retrieval을 사용한다. | plan 승인 뒤 classifier/service/response/Web을 RED→GREEN 구현 |
