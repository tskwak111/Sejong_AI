...
| A-071 | B / High | 2xx 응답 9건이 strict closed decision 전에 모두 거부된 정확한 단계 | Spec/plan approved / implementation starting | production parser typed observer의 terminal fixed enum 하나와 aggregate count만 사용한다. body/status/exception/key/DSN과 per-fixture stage는 0이다. same fixed 20·9 outbound·retry 0·USD0.20 exact-one actual은 clean committed source 뒤 1회만 실행한다. | D-108/D-109, approved spec/plan |
