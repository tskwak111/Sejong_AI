...
| A-070 | B / High | Upstage 4xx class의 정확한 운영 원인과 다음 교정 방식 | Pending / human/provider-console gate | key 활성·credit/quota·`solar-pro3` project access를 공급자 콘솔에서 사람이 확인하고, 필요하면 exact status-only 또는 provider-approved request-shape 진단 범위를 새로 승인해야 한다. | provider access/cost; 실제 재호출, 본문 보관, public/remote는 자동 승인되지 않음 |
