...
| A-072 | B / High | provider가 exact five-key closed object를 반환하도록 만드는 다음 단일 교정 | Decision A — Design section 1 review | Q-LLM-014=A/D-112로 official strict `json_schema`, required string 5필드와 fixed `NONE` sentinel을 선택했다. 서버는 sentinel을 기존 내부 `None`으로 정규화하고 closed enum/catalog 검증을 유지한다. public behavior/API/DB/data/dependency는 불변이며 상세 설계 승인 전 code/provider call 0, corrective actual은 구현·offline 검증 뒤 별도 인간 gate다. | D-111/D-112, ADR-0027, IMP-20260728-012/013 |
