...
| A-048 | A / Blocker | local 시민 chat 외부 LLM 경계 | Resolved design / written spec Review | Q-LLM-006=B·007=A·009=A·011=C·012=B: supported+masked+ACTIVE/OFFICIAL+grounded만 masked question+최소 KB 전송, `solar-pro3` 8초 1 attempt, server-issued fact ID·server-bound source, 검증 실패 전체 template fallback, `answer_mode` 배지 | D-072/ADR-0023/LLM-003 design. public/remote/실제 기관 운영 금지; 서면 명세·계획 승인 전 코드/계약/actual call 0 |
...
근거 제한형 시민 chat 생성을 승인했다. written specification과 후속 실행계획 승인 전에는
제품 코드·공개 계약 변경 또는 새 actual provider call을 하지 않는다. public/remote/실제 기관
