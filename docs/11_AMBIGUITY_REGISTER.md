...
| A-078 | D / Internal | A-077 probe-to-actual same-source·exact-lease TOCTOU 보강 | Resolved / implementation in progress | D-129로 별도 A-078 identity를 사용하고, bounded probe report와 exact lease bytes를 검증한 callback, callback 뒤 최종 source/input 재검증, probe 응답 뒤 재검증을 수행한다. D-128의 probe 1-call + 조건부 actual run 1회(정확히 9 provider calls) 권한·비용·범위는 늘리지 않는다. | D-129, ADR-0028 second amendment, A-077 specification/plan amendment, IMP-009 |
