#!/usr/bin/env python3
"""Run the isolated A-078 DeepSeek classifier pre-lease-hardened acceptance."""
...
from run_deepseek_classifier_a078_probe import (
...
A078_EVIDENCE_IDENTITY = _core.EvidenceIdentity(
    ...
    offline_result_path=_OFFLINE_DIRECTORY / "a078-offline-gate-result.json",
    offline_lock_path=_OFFLINE_DIRECTORY / "a078-offline-gate-result.json.run.lock",
    offline_stdout_path=_OFFLINE_DIRECTORY / "a078-offline-gate.stdout.log",
    offline_stderr_path=_OFFLINE_DIRECTORY / "a078-offline-gate.stderr.log",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if "--readiness-only" not in arguments:
        try:
            ...
            print("DEEPSEEK_A078_ACTUAL_PROBE_NOT_PASSED", file=sys.stderr)
            return 2
    return _core.main(arguments, evidence_identity=A078_EVIDENCE_IDENTITY)
