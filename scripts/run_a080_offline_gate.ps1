...
try {
    ...
    [Console]::Error.WriteLine("A080_OFFLINE_GATE_PREFLIGHT_FAILED")
    exit 2
}
...
try {
    ...
    if (-not $leaseAcquired) {
        [Console]::Error.WriteLine("A080_OFFLINE_GATE_LEASE_FAILED")
        exit 125
    }
    ...
    if (-not $processTerminationConfirmed) {
        [Console]::Error.WriteLine(
            "A080_OFFLINE_GATE_PROCESS_TERMINATION_UNCONFIRMED"
        )
        exit 125
    }
}
...
try {
    ...
    [Console]::Error.WriteLine("A080_OFFLINE_GATE_EVIDENCE_WRITE_FAILED")
    exit 125
}

if ($outcome -eq "PASS") {
    [Console]::Out.WriteLine("A080_OFFLINE_GATE_PASS")
    exit 0
}

[Console]::Out.WriteLine("A080_OFFLINE_GATE_FAIL")
