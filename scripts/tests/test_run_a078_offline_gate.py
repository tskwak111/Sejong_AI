...
_WRAPPER_PATH = _REPOSITORY_ROOT / "scripts" / "run_a078_offline_gate.ps1"
_RESULT_RELATIVE = Path(
    ".superpowers/sdd/2026-07-29-deepseek-prelease-hardening/"
    "a078-offline-gate-result.json"
)
_LOCK_RELATIVE = _RESULT_RELATIVE.with_name("a078-offline-gate-result.json.run.lock")
...
def _fake_repository(tmp_path: Path, *, verify_exit: int = 0) -> Path:
    repository = tmp_path / "A078 controlled repository"
    ...
    _git(repository, "config", "user.name", "A078 Test")
    _git(repository, "add", ".")
...
def test_a078_result_is_one_shot_and_disjoint(
    tmp_path: Path,
    verify_exit: int,
