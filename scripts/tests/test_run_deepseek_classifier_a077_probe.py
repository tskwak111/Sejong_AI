...
def test_probe_main_readiness_does_not_consume_lease_or_call_provider(
    ...
    monkeypatch.setattr(
        ...
        lambda _prepared: (_ for _ in ()).throw(AssertionError("provider must not be called")),
    )
...
def test_probe_main_writes_aggregate_pass_once_without_sensitive_values(
    ...
    monkeypatch.setattr(probe, "_execute_probe_with_deadline", lambda _prepared: metrics)
    ...
    assert (
        ...
        == "DEEPSEEK_A077_PROBE_RUN_ALREADY_RECORDED"
    )
