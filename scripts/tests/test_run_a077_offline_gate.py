...
_WRAPPER_PATH = _REPOSITORY_ROOT / "scripts" / "run_a077_offline_gate.ps1"
_RESULT_RELATIVE = Path(
    ".superpowers/sdd/2026-07-29-deepseek-split-timeout-correction/"
    "a077-offline-gate-result.json"
)
_LOCK_RELATIVE = _RESULT_RELATIVE.with_name("a077-offline-gate-result.json.run.lock")
...
def _fake_repository(tmp_path: Path, *, verify_exit: int = 0) -> Path:
    repository = tmp_path / "A077 controlled repository"
    ...
    _git(repository, "config", "user.name", "A077 Test")
    _git(repository, "add", ".")
...
def test_a077_result_is_one_shot_and_disjoint(
    tmp_path: Path,
    verify_exit: int,
    ...
    assert completed.stdout.strip() == f"A077_OFFLINE_GATE_{expected_outcome}"
    result_path = repository / _RESULT_RELATIVE
    result = json.loads(result_path.read_text(encoding="utf-8"))
    assert result["gate"] == "A-077-OFFLINE"
    assert result["outcome"] == expected_outcome
    assert result["invocation_count"] == 1
    assert result["rerun_count"] == 0
    assert (repository / _LOCK_RELATIVE).read_text(
        encoding="ascii"
    ) == "A-077-OFFLINE-GATE one-shot lease\n"
    artifact_root = repository / ".superpowers"
    assert not list(artifact_root.rglob("*a074*"))
    assert not list(artifact_root.rglob("*a075*"))
    assert not list(artifact_root.rglob("*a076*"))
