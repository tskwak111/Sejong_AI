...
_WRAPPER_PATH = _REPOSITORY_ROOT / "scripts" / "run_a076_offline_gate.ps1"
_RESULT_RELATIVE = Path(
    ".superpowers/sdd/2026-07-29-deepseek-network-recovery-actual/"
    "a076-offline-gate-result.json"
)
_LOCK_RELATIVE = _RESULT_RELATIVE.with_name("a076-offline-gate-result.json.run.lock")
...
def _fake_repository(tmp_path: Path, *, verify_exit: int = 0) -> Path:
    repository = tmp_path / "A076 controlled repository"
    ...
    _git(repository, "config", "user.name", "A076 Test")
    _git(repository, "add", ".")
...
def test_a076_result_is_one_shot_and_disjoint(
    tmp_path: Path,
    verify_exit: int,
