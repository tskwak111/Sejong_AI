...
$TimeoutSeconds = 3600
$PollSeconds = 1
$ResultRelativePath = ".superpowers\sdd\2026-07-29-deepseek-network-recovery-actual\a076-offline-gate-result.json"
$LockRelativePath = ".superpowers\sdd\2026-07-29-deepseek-network-recovery-actual\a076-offline-gate-result.json.run.lock"
$StdoutRelativePath = ".superpowers\sdd\2026-07-29-deepseek-network-recovery-actual\a076-offline-gate.stdout.log"
$StderrRelativePath = ".superpowers\sdd\2026-07-29-deepseek-network-recovery-actual\a076-offline-gate.stderr.log"
$LeaseText = "A-076-OFFLINE-GATE one-shot lease`n"
...
try {
    ...
    [Console]::Error.WriteLine("A076_OFFLINE_GATE_PREFLIGHT_FAILED")
    exit 2
}
...
try {
    ...
    if (-not $leaseAcquired) {
        [Console]::Error.WriteLine("A076_OFFLINE_GATE_LEASE_FAILED")
        exit 125
    }
    ...
    if (-not $processTerminationConfirmed) {
        [Console]::Error.WriteLine(
            "A076_OFFLINE_GATE_PROCESS_TERMINATION_UNCONFIRMED"
        )
        exit 125
    }
}
...
try {
    ...
    [Console]::Error.WriteLine("A076_OFFLINE_GATE_EVIDENCE_WRITE_FAILED")
    exit 125
}

if ($outcome -eq "PASS") {
    [Console]::Out.WriteLine("A076_OFFLINE_GATE_PASS")
    exit 0
}

[Console]::Out.WriteLine("A076_OFFLINE_GATE_FAIL")
