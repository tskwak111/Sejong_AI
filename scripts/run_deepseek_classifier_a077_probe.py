...
def _criteria_pass(report: Mapping[str, object]) -> bool:
    return (
        ...
        and report.get("connect_timeout_seconds")
        == DEEPSEEK_CONNECT_TIMEOUT_SECONDS
        ...
        and all(_strict_nonnegative_int(report.get(field)) for field in _NONNEGATIVE_COUNT_FIELDS)
        and _bounded_cost(
            report.get("conservative_all_miss_cost_usd_including_vat")
        )
...
async def _execute_probe(prepared: _core._PreparedRun) -> dict[str, object]:
    ...
    report: dict[str, object] = {
        ...
        "cost_cap_usd_including_vat": _core._decimal_text(
            _core._ACTUAL_COST_CAP_USD
        ),
